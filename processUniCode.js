/* this program will process data, this data represents unicode scripts */
...
var SCRIPTS = [
  ...
  let string = ' 英国的狗说 "woof", 俄罗斯的狗说 "тяв"';
  console.log(`Processing for ${string} .....`)
...
function countBy(items, categoryName) {
    let categories = [];    //array of category objects {name, count}
    for(let item of items) {
        let name = categoryName(item);
        let known = categories.findIndex(category => category.name == name);
        if(known == -1) categories.push({name, count: 1});
        else categories[known].count ++;
    }
    return categories;
}

/* test case for countBy()
console.log("counted: ", countBy([1, 2, 3, 4], n => n > 2));
//counted:  [ { name: false, count: 2 }, { name: true, count: 2 } ]
 */


/* to compute the percentage of unicode scripts used in a string */
...
// outputs: 61% Han, 22% Latin, 17% Cyrillic

/*  ************************     The most dominant writing direction in a text       *****************/
                                    /*      Helper functions     */
function createDirectionCategory(char) {
    let script = characterScript(char.codePointAt(0)); //search for a script in which the charCode lies in
    return script ? script.direction : "none";  //return the writing direction if found or "none" if not
}
function findMostWritingDirection(text) {
    let scripts = countBy(text, createDirectionCategory);
    scripts = scripts.filter(({name}) => name != "none"); //filter out the objects with the "none" category
    let maxScript = null;
    for(let script of scripts) {
        if(maxScript == null || script.count > maxScript.count) {
            maxScript = script;

        }
    }
    return maxScript ? maxScript.name : "No unicode Scripts found";
}
console.log(`The dominant writing direction is: `, findMostWritingDirection(' 英国的狗说 "woof", 俄罗斯的狗说 "тяв"'));
let ttbScripts = SCRIPTS.filter(script => script.direction == "ttb");
let ttbScriptsNames = ttbScripts.map(script => script.name);
